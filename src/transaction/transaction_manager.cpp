...
Result<LockId> DefaultLockManager::acquire_lock(
    TransactionId tx_id, const MultiLevelKey& key, LockType type,
            ...
            if (existing_lock_it != locks_.end()) {
                ...
                if (!is_lock_compatible(existing_lock.type, type)) {
                    ...
                    if (!cycle.empty()) {
                        ...
                        return Result<LockId>::error(
                            Status::DEADLOCK_DETECTED,
                            "Deadlock detected involving transaction " +
                                std::to_string(tx_id));
                    }
...
Result<void> DefaultLockManager::release_lock(const LockId& lock_id) {
    ...
    if (lock_it == locks_.end()) {
        return Result<void>::error(Status::NOT_FOUND,
    ...
    if (key_locks_it != key_locks_.end()) {
        ...
        locks_vec.erase(
            std::remove(locks_vec.begin(), locks_vec.end(), lock_id),
            locks_vec.end());
...
bool DefaultLockManager::can_acquire_lock(const MultiLevelKey& key,
                                          LockType type) const {
    std::shared_lock<std::shared_mutex> lock(mutex_);
...
std::vector<LockInfo> DefaultLockManager::get_locks_by_transaction(
    TransactionId tx_id) const {
    std::shared_lock<std::shared_mutex> lock(mutex_);
...
Result<void> DefaultLockManager::resolve_deadlock(
    const std::vector<TransactionId>& deadlock_cycle) {
    if (deadlock_cycle.empty()) {
        return Result<void>::error(Status::INVALID_ARGUMENT,
                                   "Empty deadlock cycle");
    }

    // 选择牺牲事务（这里简单选择ID最大的）
    TransactionId victim =
        *std::max_element(deadlock_cycle.begin(), deadlock_cycle.end());
...
bool DefaultLockManager::is_lock_compatible(LockType existing,
                                            LockType requested) const {
    // 锁兼容性矩阵
    switch (existing) {
        case LockType::SHARED:
            return requested == LockType::SHARED ||
                   requested == LockType::INTENTION_SHARED;
        ...
        case LockType::INTENTION_EXCLUSIVE:
            return requested == LockType::INTENTION_SHARED ||
                   requested == LockType::INTENTION_EXCLUSIVE;
        default:
            return false;
    }
}

std::vector<TransactionId> DefaultLockManager::find_cycle_in_wait_graph(
    TransactionId start) const {
    std::unordered_set<TransactionId> visited;
...
Result<void> DefaultTransactionManager::start() {
    ...
    // 启动后台线程
    cleanup_thread_ =
        std::thread(&DefaultTransactionManager::cleanup_loop, this);
    deadlock_detection_thread_ =
        std::thread(&DefaultTransactionManager::deadlock_detection_loop, this);
...
bool DefaultTransactionManager::is_running() const { return is_running_; }

Result<TransactionId> DefaultTransactionManager::begin_transaction(
    IsolationLevel isolation) {
    std::lock_guard<std::shared_mutex> lock(mutex_);

    if (!is_running_) {
        return Result<TransactionId>::error(
            Status::SERVICE_UNAVAILABLE, "Transaction manager is not running");
    }
...
Result<void> DefaultTransactionManager::commit_transaction(
    TransactionId tx_id) {
    std::unique_lock<std::shared_mutex> lock(mutex_);
    ...
    if (!all_prepared) {
        ...
        return Result<void>::error(Status::TRANSACTION_ABORTED,
...
Result<DataEntry> DefaultTransactionManager::read(TransactionId tx_id,
                                                  const MultiLevelKey& key) {
    std::shared_lock<std::shared_mutex> lock(mutex_);

    auto validate_result = validate_transaction(tx_id);
    if (!validate_result.is_ok()) {
        return Result<DataEntry>::error(validate_result.status,
    ...
    if (tx.isolation_level >= IsolationLevel::READ_COMMITTED) {
        auto lock_result = lock_manager_->acquire_lock(
            tx_id, key, LockType::SHARED, std::chrono::milliseconds(5000));
        if (!lock_result.is_ok()) {
            return Result<DataEntry>::error(lock_result.status,
...
Result<void> DefaultTransactionManager::write(TransactionId tx_id,
                                              const MultiLevelKey& key,
    ...
    // 获取排他锁
    auto lock_result = lock_manager_->acquire_lock(
        tx_id, key, LockType::EXCLUSIVE, std::chrono::milliseconds(5000));
    if (!lock_result.is_ok()) {
        return Result<void>::error(lock_result.status,
                                   lock_result.error_message);
    }
...
Result<void> DefaultTransactionManager::remove(TransactionId tx_id,
                                               const MultiLevelKey& key) {
    std::unique_lock<std::shared_mutex> lock(mutex_);
...
Result<void> DefaultTransactionManager::lock(TransactionId tx_id,
                                             const MultiLevelKey& key,
                                             LockType type) {
    std::unique_lock<std::shared_mutex> lock(mutex_);
    ...
    // 获取锁
    auto lock_result = lock_manager_->acquire_lock(
        tx_id, key, type, std::chrono::milliseconds(5000));
    if (!lock_result.is_ok()) {
        return Result<void>::error(lock_result.status,
                                   lock_result.error_message);
    }
...
Result<void> DefaultTransactionManager::handle_tpc_message(
    const TpcMessage& message) {
    std::unique_lock<std::shared_mutex> lock(mutex_);
    ...
    switch (message.type) {
        ...
        default:
            return Result<void>::error(Status::INVALID_ARGUMENT,
                                       "Unknown TPC message type");
    }

    return Result<void>(Status::OK);
}

TransactionState DefaultTransactionManager::get_transaction_state(
    TransactionId tx_id) const {
    std::shared_lock<std::shared_mutex> lock(mutex_);
...
std::vector<TransactionId> DefaultTransactionManager::get_active_transactions()
    const {
    std::shared_lock<std::shared_mutex> lock(mutex_);
...
DistributedTransaction DefaultTransactionManager::get_transaction_info(
    TransactionId tx_id) const {
    std::shared_lock<std::shared_mutex> lock(mutex_);
...
void DefaultTransactionManager::set_event_handler(
    std::shared_ptr<TransactionEventHandler> handler) {
    std::lock_guard<std::shared_mutex> lock(mutex_);
    event_handler_ = handler;
}

void DefaultTransactionManager::set_lock_manager(
    std::shared_ptr<LockManager> lock_manager) {
    std::lock_guard<std::shared_mutex> lock(mutex_);
    lock_manager_ = lock_manager;
}

void DefaultTransactionManager::set_default_timeout(
    std::chrono::milliseconds timeout) {
    std::lock_guard<std::shared_mutex> lock(mutex_);
        ...
        try {
            if (lock_manager_) {
                ...
                for (const auto& deadlock : deadlocks) {
                    ...
                    // 自动解决死锁
                    auto resolve_result =
                        lock_manager_->resolve_deadlock(deadlock);
...
Result<void> DefaultTransactionManager::validate_transaction(
    TransactionId tx_id) const {
    auto it = transactions_.find(tx_id);
    if (it == transactions_.end()) {
        return Result<void>::error(
            Status::NOT_FOUND,
            "Transaction " + std::to_string(tx_id) + " not found");
    }
    ...
    if (tx.is_expired()) {
        return Result<void>::error(
            Status::TIMEOUT,
            "Transaction " + std::to_string(tx_id) + " has expired");
    }

    return Result<void>(Status::OK);
}

Result<void> DefaultTransactionManager::apply_transaction_operations(
    const DistributedTransaction& tx) {
    // 应用事务的所有操作
...
Result<void> DefaultTransactionManager::rollback_transaction_operations(
    const DistributedTransaction& tx) {
    // 回滚事务的所有操作（逆序执行）
...
Result<void> DefaultTransactionManager::send_prepare_messages(
    TransactionId tx_id) {
    auto tx_it = transactions_.find(tx_id);
...
Result<void> DefaultTransactionManager::send_commit_messages(
    TransactionId tx_id) {
    auto tx_it = transactions_.find(tx_id);
    if (tx_it == transactions_.end()) {
        return Result<void>::error(Status::NOT_FOUND, "Transaction not found");
...
Result<void> DefaultTransactionManager::send_abort_messages(
    TransactionId tx_id) {
    auto tx_it = transactions_.find(tx_id);
    if (tx_it == transactions_.end()) {
        return Result<void>::error(Status::NOT_FOUND, "Transaction not found");
...
void DefaultTransactionManager::handle_prepare_message(
    const TpcMessage& message) {
    // 作为参与者处理准备消息
...
void DefaultTransactionManager::handle_prepare_response(
    const TpcMessage& message) {
    // 作为协调者处理准备响应
    auto tx_id = message.transaction_id;
...
void DefaultTransactionManager::handle_commit_message(
    const TpcMessage& message) {
    // 作为参与者处理提交消息
    auto tx_id = message.transaction_id;
...
void DefaultTransactionManager::handle_abort_message(
    const TpcMessage& message) {
    // 作为参与者处理回滚消息
    auto tx_id = message.transaction_id;
...
void DefaultTransactionManager::notify_transaction_started(
    TransactionId tx_id) {
    if (event_handler_) {
...
void DefaultTransactionManager::notify_transaction_committed(
    TransactionId tx_id) {
    if (event_handler_) {
...
void DefaultTransactionManager::notify_transaction_aborted(
    TransactionId tx_id, const std::string& reason) {
    if (event_handler_) {
        event_handler_->on_transaction_aborted(tx_id, reason);
    }
}

void DefaultTransactionManager::notify_deadlock_detected(
    const std::vector<TransactionId>& deadlock_cycle) {
    if (event_handler_) {
...
TransactionContext::TransactionContext(
    std::shared_ptr<TransactionManager> manager, TransactionId tx_id)
    : manager_(manager),
...
Result<DataEntry> TransactionContext::read(const MultiLevelKey& key) {
    if (is_committed_ || is_aborted_) {
        return Result<DataEntry>::error(Status::INVALID_STATE,
...
Result<void> TransactionContext::write(const MultiLevelKey& key,
                                       const DataEntry& value) {
    if (is_committed_ || is_aborted_) {
        return Result<void>::error(Status::INVALID_STATE,
                                   "Transaction is already finished");
    }
