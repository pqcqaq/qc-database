...
void print_usage(const char* program_name) {
    std::cout
        ...
        << "  " << program_name
        << " --node-id node1 --seeds node2:8080,node3:8080\n\n";
}

void print_version() {
    std::cout
        << "Distributed Cache Server v1.0.0\n"
...
int main(int argc, char* argv[]) {
    ...
    while ((c = getopt_long(argc, argv, "c:p:a:d:n:s:l:Dhv", long_options,
                            &option_index)) != -1) {
        switch (c) {
            case 'c':
                config_file = optarg;
    ...
    try {
        ...
        if (!config_file.empty()) {
            ...
            if (config_result.is_ok()) {
                ...
                std::cout << "Could not load config file, using default "
                             "configuration\n";
            }
        }
        ...
        if (config.enable_monitoring) {
            std::cout << "  - Metrics endpoint: http://"
                      << config.listen_address << ":" << config.metrics_port
                      << "/metrics\n";
        }

        std::cout << "\nServer is ready to accept connections. Press Ctrl+C to "
                     "stop.\n";
