...
bool WALEntry::verify_checksum() const {
    ...
    auto calculated = crc32c::Crc32c(entry_copy.serialize().c_str(),
                                     entry_copy.serialize().size());
...
Result<WALEntry> WALEntry::deserialize(const std::string& data) {
    ...
    // Deserialize type
    if (pos >= data.size()) {
        return Result<WALEntry>::error(Status::INVALID_ARGUMENT,
                                       "Invalid data format");
    }
...
Result<void> WAL::open() {
    ...
    try {
        ...
        if (!file_->is_open()) {
            ...
            if (!file_->is_open()) {
                return Result<void>::error(
                    ...
                    "Failed to create WAL file: " + wal_file_);
            }
            file_->close();
            file_->open(wal_file_,
                        std::ios::binary | std::ios::in | std::ios::out);
        }

        if (!file_->is_open()) {
            return Result<void>::error(Status::STORAGE_ERROR,
                                       "Failed to open WAL file: " + wal_file_);
        }
        ...
        return Result<void>::error(
            Status::STORAGE_ERROR,
            "Failed to open WAL: " + std::string(e.what()));
    }
}
...
Result<void> WAL::close() {
    ...
    try {
        ...
        return Result<void>::error(
            Status::STORAGE_ERROR,
            "Failed to close WAL: " + std::string(e.what()));
    }
}
...
Result<void> WAL::append(const WALEntry& entry) {
    ...
    try {
        ...
        uint32_t crc =
            crc32c::Crc32c(serialized_entry.c_str(), serialized_entry.size());

        file_->write(reinterpret_cast<const char*>(&record_length),
        ...
        return Result<void>::error(
            Status::STORAGE_ERROR,
            "Failed to append to WAL: " + std::string(e.what()));
    }
}
...
Result<void> WAL::sync() {
    ...
    try {
        ...
        return Result<void>::error(
            Status::STORAGE_ERROR,
            "Failed to sync WAL: " + std::string(e.what()));
    }
}
...
Result<std::vector<WALEntry>> WAL::replay_from(uint64_t sequence_number) {
    ...
    try {
        ...
        while (file_->good()) {
            ...
            file_->read(reinterpret_cast<char*>(&record_length),
                        sizeof(record_length));
            if (file_->gcount() != sizeof(record_length)) {
                break;
            }

            file_->read(reinterpret_cast<char*>(&expected_crc),
                        sizeof(expected_crc));
            ...
            if (record_length > 1024 * 1024) {  // Max 1MB per record
                return Result<std::vector<WALEntry>>::error(
                    Status::STORAGE_ERROR, "Invalid record length in WAL: " +
            ...
            if (static_cast<uint32_t>(file_->gcount()) != record_length) {
                return Result<std::vector<WALEntry>>::error(
                    Status::STORAGE_ERROR, "Incomplete record in WAL");
            }

            uint32_t actual_crc =
                crc32c::Crc32c(record_data.c_str(), record_data.size());
            if (actual_crc != expected_crc) {
                return Result<std::vector<WALEntry>>::error(
                    Status::STORAGE_ERROR, "CRC mismatch in WAL record");
            }

            auto deserialize_result = WALEntry::deserialize(record_data);
            if (!deserialize_result.is_ok()) {
                return Result<std::vector<WALEntry>>::error(
                    Status::STORAGE_ERROR, "Failed to deserialize WAL entry");
            }
...
Result<void> WAL::truncate(uint64_t sequence_number) {
    ...
    try {
        ...
        std::ofstream output_file(wal_file_,
        ...
        for (const auto& entry : replay_result.data) {
            ...
            uint32_t crc =
                crc32c::Crc32c(serialized.c_str(), serialized.size());

            output_file.write(reinterpret_cast<const char*>(&record_length),
                              sizeof(record_length));
        ...
        if (!file_->is_open()) {
            return Result<void>::error(Status::STORAGE_ERROR,
                                       "Failed to reopen WAL after truncation");
        }
        ...
        return Result<void>::error(
            Status::STORAGE_ERROR,
            "Failed to truncate WAL: " + std::string(e.what()));
    }
}
