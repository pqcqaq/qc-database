...
Result<void> LSMTreeEngine::open() {
    ...
    try {
        ...
        return Result<void>::error(
            ...
            "Failed to open LSM tree: " + std::string(e.what()));
    }
}
...
Result<void> LSMTreeEngine::close() {
    ...
    try {
        ...
        return Result<void>::error(
            Status::STORAGE_ERROR,
            "Failed to close LSM tree: " + std::string(e.what()));
    }
}

bool LSMTreeEngine::is_open() const { return is_open_; }
...
Result<void> LSMTreeEngine::put(const MultiLevelKey& key,
...
Result<void> LSMTreeEngine::put_if_version_match(const MultiLevelKey& key,
    ...
    if (current_result.is_ok()) {
        if (current_result.data.version != expected_version) {
            return Result<void>::error(Status::VERSION_CONFLICT,
                                       "Version mismatch");
        }
    } else if (expected_version != 0) {
        return Result<void>::error(
            Status::VERSION_CONFLICT,
            "Key does not exist but expected version is not 0");
    }
...
LSMTreeEngine::range_scan(const RangeQuery& query) {
    ...
    return Result<std::vector<std::pair<MultiLevelKey, DataEntry>>>(
        Status::OK, final_results);
}

Result<std::vector<DataEntry>> LSMTreeEngine::multi_get(
    const std::vector<MultiLevelKey>& keys) {
...
Result<void> LSMTreeEngine::multi_put(
    const std::vector<std::pair<MultiLevelKey, DataEntry>>& entries) {
...
Result<void> LSMTreeEngine::backup(const std::string& backup_path) {
    ...
    try {
        ...
        std::filesystem::copy_file(
            data_dir_ + "/wal.log", backup_path + "/wal.log",
        ...
        for (const auto& sstable : sstables_) {
            ...
            std::filesystem::path dst_path =
                std::filesystem::path(backup_path) / src_path.filename();
            std::filesystem::copy_file(
                src_path, dst_path,
                std::filesystem::copy_options::overwrite_existing);
        }
        ...
        return Result<void>::error(
            Status::STORAGE_ERROR,
            "Failed to backup: " + std::string(e.what()));
    }
}
...
Result<void> LSMTreeEngine::restore(const std::string& backup_path) {
    ...
    if (is_open_) {
        return Result<void>::error(Status::STORAGE_ERROR,
                                   "Cannot restore while LSM tree is open");
    }
    ...
    try {
        ...
        for (const auto& entry :
             std::filesystem::directory_iterator(backup_path)) {
        ...
        return Result<void>::error(
            Status::STORAGE_ERROR,
            "Failed to restore: " + std::string(e.what()));
    }
}
...
Result<void> LSMTreeEngine::flush_memtable() {
    ...
    try {
        ...
        // 3. 写入WAL检查点
        WALEntry checkpoint_entry(WALEntryType::CHECKPOINT, MultiLevelKey(),
                                  DataEntry(), 0);
        ...
        return Result<void>::error(
            Status::STORAGE_ERROR,
            "Failed to flush memtable: " + std::string(e.what()));
    }
}
...
Result<void> LSMTreeEngine::compact_sstables() {
    ...
    try {
        ...
        return Result<void>::error(
            Status::STORAGE_ERROR,
            "Failed to compact SSTables: " + std::string(e.what()));
    }
}
...
Result<void> LSMTreeEngine::recover_from_wal() {
    ...
    if (!replay_result.is_ok()) {
        return Result<void>::error(replay_result.status,
...
Result<void> LSMTreeEngine::load_sstables() {
    try {
        ...
        for (const auto& entry :
             std::filesystem::directory_iterator(data_dir_)) {
        ...
        return Result<void>::error(
            Status::STORAGE_ERROR,
            "Failed to load SSTables: " + std::string(e.what()));
    }
}
...
std::string LSMTreeEngine::generate_sstable_filename() {
    ...
    oss << data_dir_ << "/sstable_"
        ...
        << std::setfill('0') << std::setw(3) << ms.count() << ".sst";
