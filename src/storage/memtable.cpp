...
Result<void> MemTable::put(const MultiLevelKey& key, const DataEntry& entry) {
    ...
    size_t new_entry_size =
        key.to_string().size() + entry.value.size() + sizeof(DataEntry);
    size_t old_entry_size = 0;

    auto it = data_.find(key);
    if (it != data_.end()) {
        old_entry_size = key.to_string().size() + it->second.value.size() +
                         sizeof(DataEntry);
    }
...
Result<std::vector<std::pair<MultiLevelKey, DataEntry>>> MemTable::range_scan(
    ...
    if (!query.inclusive_start && start_it != data_.end() &&
        start_it->first == query.start_key) {
        ++start_it;
    }

    if (query.inclusive_end && end_it != data_.end() &&
        end_it->first == query.end_key) {
        ++end_it;
    }
    ...
    return Result<std::vector<std::pair<MultiLevelKey, DataEntry>>>(Status::OK,
                                                                    results);
}
...
size_t MemTable::memory_usage() const { return memory_usage_.load(); }
