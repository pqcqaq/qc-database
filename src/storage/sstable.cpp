    ...
    try {
        ...
        if (!file.is_open()) {
            return Result<void>::error(
                ...
                "Failed to open SSTable file for writing: " + filename_);
        }
        ...
        file.write(reinterpret_cast<const char*>(&magic_number),
                   sizeof(magic_number));
        file.write(reinterpret_cast<const char*>(&version), sizeof(version));
        file.write(reinterpret_cast<const char*>(&entry_count),
                   sizeof(entry_count));
        ...
        for (const auto& [key, entry] : data) {
            ...
            file.write(reinterpret_cast<const char*>(&key_size),
                       sizeof(key_size));
            file.write(key_str.c_str(), key_size);

            // 序列化数据
            uint32_t value_size = entry.value.size();
            file.write(reinterpret_cast<const char*>(&value_size),
                       sizeof(value_size));
            file.write(entry.value.c_str(), value_size);
            file.write(reinterpret_cast<const char*>(&entry.version),
                       sizeof(entry.version));

            auto timestamp_ms = entry.timestamp.count();
            file.write(reinterpret_cast<const char*>(&timestamp_ms),
                       sizeof(timestamp_ms));

            uint8_t deleted_flag = entry.deleted ? 1 : 0;
            file.write(reinterpret_cast<const char*>(&deleted_flag),
                       sizeof(deleted_flag));

            index_entry.size =
                static_cast<uint64_t>(file.tellp()) - current_offset;
        ...
        file.write(reinterpret_cast<const char*>(&index_size),
                   sizeof(index_size));
        ...
        for (const auto& entry : index_) {
            ...
            file.write(reinterpret_cast<const char*>(&entry.offset),
                       sizeof(entry.offset));
            file.write(reinterpret_cast<const char*>(&entry.size),
                       sizeof(entry.size));
        }

        // 写入尾部信息（索引偏移）
        file.write(reinterpret_cast<const char*>(&index_offset),
                   sizeof(index_offset));
        ...
        return Result<void>::error(
            Status::STORAGE_ERROR,
            "Failed to write SSTable: " + std::string(e.what()));
    }
}
...
Result<DataEntry> SSTable::read(const MultiLevelKey& key) {
    ...
    if (index_.empty()) {
        ...
        if (!load_result.is_ok()) {
            return Result<DataEntry>::error(load_result.status,
    ...
    // 二分查找索引
    auto it =
        std::lower_bound(index_.begin(), index_.end(), key,
                         [](const IndexEntry& entry, const MultiLevelKey& k) {
                             return entry.key < k;
                         });

    if (it == index_.end() || it->key != key) {
        return Result<DataEntry>::error(Status::NOT_FOUND,
    ...
    try {
        ...
        if (read_key != key.to_string()) {
            return Result<DataEntry>::error(Status::STORAGE_ERROR,
                                            "Key mismatch in SSTable");
        }
        ...
        file.read(reinterpret_cast<char*>(&entry.version),
                  sizeof(entry.version));
        ...
        return Result<DataEntry>::error(
            Status::STORAGE_ERROR,
            "Failed to read from SSTable: " + std::string(e.what()));
    }
}

Result<std::vector<std::pair<MultiLevelKey, DataEntry>>> SSTable::range_scan(
    ...
    if (index_.empty()) {
        ...
        if (!load_result.is_ok()) {
            return Result<std::vector<std::pair<MultiLevelKey, DataEntry>>>::
                error(load_result.status, load_result.error_message);
        }
    }
    ...
    try {
        ...
        auto start_it = std::lower_bound(
            index_.begin(), index_.end(), query.start_key,
            [](const IndexEntry& entry, const MultiLevelKey& k) {
                return entry.key < k;
            });

        auto end_it = std::upper_bound(
            index_.begin(), index_.end(), query.end_key,
            [](const MultiLevelKey& k, const IndexEntry& entry) {
                return k < entry.key;
            });

        if (!query.inclusive_start && start_it != index_.end() &&
            start_it->key == query.start_key) {
            ++start_it;
        }
        ...
        uint32_t count = 0;
        for (auto it = start_it; it != end_it && count < query.limit;
             ++it, ++count) {
            // 读取数据
            file.seekg(it->offset);
            ...
            uint64_t timestamp_ms;
            file.read(reinterpret_cast<char*>(&timestamp_ms),
                      sizeof(timestamp_ms));
            entry.timestamp = Timestamp(timestamp_ms);

            uint8_t deleted_flag;
            file.read(reinterpret_cast<char*>(&deleted_flag),
                      sizeof(deleted_flag));
            entry.deleted = (deleted_flag != 0);
        ...
        return Result<std::vector<std::pair<MultiLevelKey, DataEntry>>>::error(
            Status::STORAGE_ERROR,
            "Failed to scan SSTable: " + std::string(e.what()));
    }
}

Result<void> SSTable::build_index() { return load_index(); }
...
Result<void> SSTable::load_index() {
    try {
        ...
        for (uint32_t i = 0; i < index_size; ++i) {
            ...
            // 读取偏移和大小
            file.read(reinterpret_cast<char*>(&entry.offset),
                      sizeof(entry.offset));
        ...
        return Result<void>::error(
            Status::STORAGE_ERROR,
            "Failed to load SSTable index: " + std::string(e.what()));
    }
}
