...
Result<void> DefaultSplitBrainDetector::start(const SplitBrainConfig& config) {
    ...
    // 启动检测线程
    detection_thread_ =
        std::thread(&DefaultSplitBrainDetector::detection_loop, this);
    recovery_thread_ =
        std::thread(&DefaultSplitBrainDetector::recovery_loop, this);
...
bool DefaultSplitBrainDetector::is_running() const { return is_running_; }
...
void DefaultSplitBrainDetector::update_node_status(
    const NodeId& node_id, bool is_alive,
...
void DefaultSplitBrainDetector::process_heartbeat(
    const NodeId& from_node, const NodeId& to_node,
...
std::vector<NetworkPartition>
DefaultSplitBrainDetector::get_detected_partitions() const {
    std::shared_lock<std::shared_mutex> lock(mutex_);
...
Result<void> DefaultSplitBrainDetector::isolate_node(
    const NodeId& node_id, const std::string& reason) {
    std::lock_guard<std::shared_mutex> lock(mutex_);
...
void DefaultSplitBrainDetector::set_event_handler(
    std::shared_ptr<SplitBrainEventHandler> handler) {
    std::lock_guard<std::shared_mutex> lock(mutex_);
...
void DefaultSplitBrainDetector::handle_partition_detected(
    const NetworkPartition& partition) {
    notify_split_brain_detected(partition);
}

void DefaultSplitBrainDetector::handle_partition_resolved(
    const NetworkPartition& partition) {
...
QuorumDecision DefaultSplitBrainDetector::dynamic_quorum() const {
    ...
    size_t dynamic_quorum_size =
        std::max(config_.min_quorum_size, (alive_nodes.size() + 1) / 2);
...
QuorumDecision DefaultSplitBrainDetector::witness_based_quorum() const {
    ...
    for (const auto& [node_id, status] : nodes_) {
        if (status.is_alive && !status.is_isolated) {
            ...
            // 检查是否是见证者节点
            if (std::find(config_.witness_nodes.begin(),
                          config_.witness_nodes.end(),
    ...
    decision.active_nodes = alive_nodes;
    decision.reason =
        "Witness-based quorum: " + std::to_string(alive_witnesses.size()) +
...
DefaultSplitBrainDetector::find_connected_components() const {
...
bool DefaultSplitBrainDetector::are_nodes_connected(const NodeId& node1,
    ...
    bool node1_recent =
        (now - it1->second.last_heartbeat) < heartbeat_threshold;
    bool node2_recent =
        (now - it2->second.last_heartbeat) < heartbeat_threshold;
        ...
        if (!partition.resolved) {
            ...
            for (size_t i = 0; i < partition.partition_a.size(); i++) {
                for (size_t j = i + 1; j < partition.partition_a.size(); j++) {
                    if (!are_nodes_connected(partition.partition_a[i],
                                             partition.partition_a[j])) {
            ...
            for (size_t i = 0; i < partition.partition_b.size(); i++) {
                for (size_t j = i + 1; j < partition.partition_b.size(); j++) {
                    if (!are_nodes_connected(partition.partition_b[i],
                                             partition.partition_b[j])) {
...
void DefaultSplitBrainDetector::notify_split_brain_detected(
    const NetworkPartition& partition) {
    if (event_handler_) {
...
void DefaultSplitBrainDetector::notify_split_brain_resolved(
    const NetworkPartition& partition) {
    if (event_handler_) {
...
void DefaultSplitBrainDetector::notify_node_isolated(
    const NodeId& node_id, const std::string& reason) {
    if (event_handler_) {
...
std::vector<std::vector<bool>> calculate_connectivity_matrix(
    ...
    const std::function<bool(const NodeId&, const NodeId&)>&
...
size_t calculate_required_quorum_size(size_t total_nodes,
...
bool validate_quorum_decision(const QuorumDecision& decision,
    ...
    // 验证决策的一致性
    size_t total_accounted =
        decision.active_nodes.size() + decision.isolated_nodes.size();
