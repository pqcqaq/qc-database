...
HybridLogicalClock HybridLogicalClock::from_string(const std::string& str) {
    ...
    if (pos != std::string::npos) {
        ...
        hlc.physical_time = std::chrono::system_clock::from_time_t(std::mktime(&tm));
    }
...
DefaultTimeService::DefaultTimeService() 
    : is_running_(false), stop_threads_(false), local_offset_(0),
      logical_time_(0), sync_attempts_(0), sync_successes_(0), sync_failures_(0) {
...
bool DefaultTimeService::is_running() const {
    return is_running_;
}
...
HybridLogicalClock DefaultTimeService::update_logical_clock(const HybridLogicalClock& received) {
    std::lock_guard<std::shared_mutex> lock(mutex_);
...
Result<void> DefaultTimeService::set_offset(const NodeId& node_id, ClockOffset offset) {
    std::lock_guard<std::shared_mutex> lock(mutex_);
...
void DefaultTimeService::set_event_handler(std::shared_ptr<TimeSyncEventHandler> handler) {
    std::lock_guard<std::shared_mutex> lock(mutex_);
...
std::chrono::milliseconds DefaultTimeService::get_estimated_uncertainty() const {
    std::shared_lock<std::shared_mutex> lock(mutex_);
...
Result<ClockOffset> DefaultTimeService::measure_offset_with_node(const NodeId& node_id) {
    // 简化的时间偏移测量
    // 实际实现需要通过网络与目标节点通信
    ...
    auto offset = std::chrono::duration_cast<std::chrono::milliseconds>(remote_time - local_time);
    
    return Result<ClockOffset>(Status::OK, offset);
}

Result<ClockOffset> DefaultTimeService::sync_with_ntp_server(const std::string& ntp_server) {
    // 简化的NTP客户端实现
    // 实际实现需要完整的NTP协议
...
void DefaultTimeService::update_offset_with_sample(const NodeId& node_id, ClockOffset sample, 
...
ClockOffset DefaultTimeService::filter_offset(ClockOffset current, ClockOffset new_sample) const {
    // 指数移动平均滤波
    double alpha = config_.offset_filter_alpha;
    auto filtered_ms = (1.0 - alpha) * current.count() + alpha * new_sample.count();
...
Result<ClockOffset> DefaultTimeService::query_ntp_server(const std::string& server) {
    // 简化的NTP查询实现
    ...
    try {
        ...
        // 模拟NTP服务器返回的时间偏移
        // 实际情况下这里会是真实的网络通信
        auto simulated_offset = ClockOffset(std::rand() % 100 - 50); // -50ms to +50ms
...
void DefaultTimeService::handle_time_response(const TimeSyncMessage& message) {
    ...
    auto offset = std::chrono::duration_cast<std::chrono::milliseconds>(remote_time - local_time);
    auto uncertainty = std::chrono::duration_cast<std::chrono::milliseconds>(rtt / 4);
...
void DefaultTimeService::notify_clock_drift_detected(const NodeId& node_id, ClockOffset drift) {
    if (event_handler_) {
        event_handler_->on_clock_drift_detected(node_id, drift);
    }
}

void DefaultTimeService::notify_time_sync_completed(const NodeId& node_id, ClockOffset offset) {
    if (event_handler_) {
        event_handler_->on_time_sync_completed(node_id, offset);
    }
}

void DefaultTimeService::notify_time_sync_failed(const NodeId& node_id, const std::string& reason) {
    if (event_handler_) {
...
bool TimestampOrdering::is_before(const HybridLogicalClock& ts1, const HybridLogicalClock& ts2) const {
    return ts1 < ts2;
}

bool TimestampOrdering::is_concurrent(const HybridLogicalClock& ts1, const HybridLogicalClock& ts2) const {
    // 如果物理时间差很小且逻辑时间相近，认为是并发的
    auto time_diff = std::abs(std::chrono::duration_cast<std::chrono::milliseconds>(
        ts1.physical_time - ts2.physical_time).count());
    auto logical_diff = std::abs(static_cast<int64_t>(ts1.logical_time - ts2.logical_time));
    
    return time_diff < 100 && logical_diff < 10; // 100ms内且逻辑时间差小于10
}

Result<void> TimestampOrdering::wait_for_timestamp(const HybridLogicalClock& target_ts, 
...
double calculate_sync_quality(const std::vector<ClockOffsetInfo>& offsets,
    ...
    for (const auto& offset : offsets) {
        ...
        double uncertainty_factor = 1.0 - static_cast<double>(offset.uncertainty.count()) / 
...
uint64_t physical_to_ntp_timestamp(PhysicalTimestamp timestamp) {
    ...
    auto ntp_epoch = unix_epoch - std::chrono::seconds(2208988800ULL); // 1900-1970差异
    
    auto duration_since_ntp_epoch = timestamp - ntp_epoch;
    auto seconds = std::chrono::duration_cast<std::chrono::seconds>(duration_since_ntp_epoch);
    auto fraction = duration_since_ntp_epoch - seconds;
    
    uint64_t ntp_seconds = seconds.count();
    uint64_t ntp_fraction = std::chrono::duration_cast<std::chrono::nanoseconds>(fraction).count() * 
...
PhysicalTimestamp ntp_to_physical_timestamp(uint64_t ntp_timestamp) {
    ...
    uint64_t nanoseconds = (static_cast<uint64_t>(fraction) * 1000000000ULL) >> 32;
    
    auto unix_epoch = std::chrono::system_clock::from_time_t(0);
    return unix_epoch + std::chrono::seconds(unix_seconds) + std::chrono::nanoseconds(nanoseconds);
}
